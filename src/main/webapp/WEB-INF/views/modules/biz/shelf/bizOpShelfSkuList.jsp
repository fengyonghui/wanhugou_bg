...
<body>
	...
	<form:form id="searchForm" modelAttribute="bizOpShelfSku" action="${ctx}/biz/shelf/bizOpShelfSku/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			<li><label>上架人：</label>
				<form:input path="shelfUser" htmlEscape="false" maxlength="11" class="input-medium"/>
			</li>
			<li><label>上架时间：</label>
				<input name="shelfTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${bizOpShelfSku.shelfTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
			<li><label>下架人：</label>
				<form:input path="unshelfUser" htmlEscape="false" maxlength="11" class="input-medium"/>
			</li>
			<li><label>下架时间：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				<th>上架商品ID</th>
				<th>采购中心ID（sys_office.id） 0:代表平台商品</th>
				<th>上架人</th>
				<th>上架数量</th>
				<th>原价</th>
				<th>销售单价-现价</th>
				<th>此单价所对应的最低销售数量；</th>
				<th>此单价所对应的最高销售数量；0：不限制</th>
				<th>上架时间</th>
				<th>下架人</th>
				<th>下架时间</th>
				<th>显示次序</th>
		...
		<c:forEach items="${page.list}" var="bizOpShelfSku">
			<tr>
				...
				<td>
					${bizOpShelfSku.centerId}
				</td>
				<td>
					${bizOpShelfSku.shelfUser}
				</td>
				<td>
