...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/biz/shelf/bizOpShelfSku/cendList">商品上架列表</a></li>
		<shiro:hasPermission name="biz:shelf:bizOpShelfSku:edit"><li><a href="${ctx}/biz/shelf/bizOpShelfSku/cendForm?shelfSign=0">商品上架添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="bizOpShelfSku" action="${ctx}/biz/shelf/bizOpShelfSku/cendList" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bizOpShelfSku">
			<tr>
				...
				<td><a href="${ctx}/biz/shelf/bizOpShelfSku/cendForm?id=${bizOpShelfSku.id}">
					${bizOpShelfSku.skuInfo.name}
				</a></td>
				<td>
					<%--<a href="${ctx}/biz/product/bizProductInfoV2/cendForm?id=${bizOpShelfSku.productInfo.id}">--%>
                ...
                <shiro:hasPermission name="biz:shelf:bizOpShelfSku:edit"><td>
					<a href="${ctx}/biz/shelf/bizOpShelfSku/cendForm?id=${bizOpShelfSku.id}">修改</a>
					<c:choose>
						<c:when test="${bizOpShelfSku.udshelf eq '上架'}">
						    <a href="${ctx}/biz/shelf/bizOpShelfSku/shelvesSave?id=${bizOpShelfSku.id}&cendShelf=cend_save" onclick="return confirm('确认要上架该商品吗？', this.href)">上架</a>
						</c:when>
						<c:otherwise>
							<a href="${ctx}/biz/shelf/bizOpShelfSku/dateTimeSave?id=${bizOpShelfSku.id}&cendShelf=cend_save" onclick="return confirm('确认要下架该商品吗？', this.href)">下架</a>
						</c:otherwise>
					</c:choose>
					<a href="${ctx}/biz/shelf/bizOpShelfSku/delete?id=${bizOpShelfSku.id}&shelfSign=0&cendShelf=cend_save" onclick="return confirmx('确认要删除该上架商品吗？', this.href)">删除</a>
