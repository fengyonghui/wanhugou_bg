	...
	<script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#confirm").click(function () {
                var total = $("#total").val();
                var realMoney = $("#realMoney").val();
                if (parseInt(total) < parseInt(realMoney)) {
                    alert("已超出应收金额");
                    return false;
                }
                if (confirm("请再次确认实收金额是否正确？")==true) {
                    ...
                    $.ajax({
                        ...
                        data:{id:orderHeaderId,realMoney:realMoney},
...
<ul class="nav nav-tabs">
	...
	<c:if test="${empty bizOrderHeader.flag}">
		<c:if test="${empty bizOrderHeader.clientModify}">
			<li><a href="${ctx}/biz/order/bizOrderHeader?statu=${bizOrderHeader.statu}">订单信息列表</a></li>
		</c:if>
	...
	<li class="active">
		...
		<c:if test="${entity.orderDetails eq 'details'}">
			<a href="${ctx}/biz/order/bizOrderHeader/form?id=${bizOrderHeader.id}&orderDetails=${entity.orderDetails}&statu=${bizOrderHeader.statu}">订单信息详情</a>
		</c:if>
		...
		<c:if test="${empty entity.orderNoEditable && empty bizOrderHeader.flag && empty entity.orderDetails}">
			<c:if test="${empty bizOrderHeader.clientModify}">
				<a href="${ctx}/biz/order/bizOrderHeader/form?id=${bizOrderHeader.id}&statu=${bizOrderHeader.statu}">订单信息<shiro:hasPermission
...
<form:form id="inputForm" modelAttribute="bizOrderHeader" action="${ctx}/biz/order/bizOrderHeader/save" method="post"
	...
	<c:if test="${entity.orderNoEditable eq 'editable' || entity.orderDetails eq 'details' || bizOrderHeader.flag eq 'check_pending'}">
		<div class="control-group">
			...
			<div class="controls">
				<input id="total" type="text" value="<fmt:formatNumber type="number" value="${bizOrderHeader.totalDetail+bizOrderHeader.totalExp+bizOrderHeader.freight}" pattern="0.00"/>"
	...
	<c:if test="${not empty entity.orderDetails}">
		...
		<div class="control-group">
			...
			<div class="controls">
                <input id="unlinePayMoney" disabled="disabled" class="input-mini" value="${bizOrderHeader.orderHeaderUnline.unlinePayMoney}"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">实收金额：</label>
			<div class="controls">
				<input id="realMoney" class="input-mini" value="${bizOrderHeader.orderHeaderUnline.realMoney==null?0:bizOrderHeader.orderHeaderUnline.realMoney}"/>
