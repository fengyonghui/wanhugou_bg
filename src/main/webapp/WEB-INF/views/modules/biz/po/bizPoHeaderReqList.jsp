		...
		$(document).ready(function() {
            $("#buttonExport").click(function(){
                top.$.jBox.confirm("确认要导出订单数据吗？","系统提示",function(v,h,f){
                    if(v=="ok"){
                        $("#searchForm").attr("action","${ctx}/biz/po/bizPoHeaderReq/poHeaderExport");
                        $("#searchForm").submit();
                        $("#searchForm").attr("action","${ctx}/biz/po/bizPoHeaderReq/");
                    }
                },{buttonsFocus:1});
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/biz/po/bizPoHeaderReq/">采购订单列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizPoHeader" action="${ctx}/biz/po/bizPoHeaderReq/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bizPoHeader" varStatus="state">
			<tr>
				<td>${state.index+1}</td>
				<td><a href="${ctx}/biz/po/bizPoHeaderReq/form?id=${bizPoHeader.id}&str=detail">
					...
					<c:choose>
						...
						<c:otherwise>
							<c:if test="${bizPoHeader.schedulingType == 0}">
								<c:choose>
									<c:when test="${bizPoHeader.totalSchedulingHeaderNum == null || bizPoHeader.totalSchedulingHeaderNum == 0}">
										${BizOrderSchedulingEnum.SCHEDULING_NOT.desc}
									</c:when>
									<c:otherwise>
										<c:if test="${bizPoHeader.totalOrdQty != bizPoHeader.totalSchedulingHeaderNum}">
					...
					<td>
						<c:if test="${bizPoHeader.bizStatus != 10}">
							...
							&& (fns:hasRole(roleSet, bizPoHeader.commonProcess.purchaseOrderProcess.roleEnNameEnum) || fns:getUser().isAdmin())
									...
									<a href="${ctx}/biz/po/bizPoHeaderReq/form?id=${bizPoHeader.id}&type=audit">审核</a>
								</c:if>
							...
							<%--</shiro:hasPermission>--%>
							<shiro:hasPermission name="biz:po:bizPoHeader:view">
								<a href="${ctx}/biz/po/bizPoHeaderReq/form?id=${bizPoHeader.id}&str=detail">详情</a>
							</shiro:hasPermission>
							<c:if test="${bizPoHeader.commonProcess.purchaseOrderProcess.name == '审批完成'}">
								<c:if test="${bizPoHeader.totalOrdQty != null && bizPoHeader.totalOrdQty != 0}">
									<shiro:hasPermission name="biz:po:bizPoHeader:addScheduling">
										<a href="${ctx}/biz/po/bizPoHeaderReq/scheduling?id=${bizPoHeader.id}">排产</a>
									</shiro:hasPermission>
									<shiro:hasPermission name="biz:po:bizPoHeader:confirmScheduling">
										<a href="${ctx}/biz/po/bizPoHeaderReq/scheduling?id=${bizPoHeader.id}&forward=confirmScheduling">确认排产</a>
									</shiro:hasPermission>
								</c:if>
							</c:if>
						</c:if>
...
<script type="text/javascript">
    function cancel(id){
        top.$.jBox.confirm("确认要取消吗？","系统提示",function(v,h,f){
            if(v=="ok"){
                $.ajax({
                    url:"${ctx}/biz/po/bizPoHeaderReq/cancel?id=" + id,
                    type:"post",
                    cache:false,
