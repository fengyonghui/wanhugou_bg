...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/biz/pay/bizPayRecord/CendList">交易记录列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="bizPayRecord" action="${ctx}/biz/pay/bizPayRecord/CendList" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>业务单号：</label>
				<form:input path="outTradeNo" htmlEscape="false" placeholder="微信/支付宝 支付业务单号" maxlength="50" class="input-medium"/>
			</li>
			<li><label>收支金额：</label>
			...
			<tr>
				...
				<shiro:hasPermission name="biz:pay:bizPayRecord:edit"><td>
					...
					<c:if test="${bizPayRecord.delFlag!=null && bizPayRecord.delFlag!=0}">
						<a href="${ctx}/biz/pay/bizPayRecord/delete?id=${bizPayRecord.id}&cendDele=end_dele" onclick="return confirmx('确认要删除该交易记录吗？', this.href)">删除</a>
					</c:if>
					<c:if test="${bizPayRecord.delFlag!=null && bizPayRecord.delFlag==0}">
						<a href="${ctx}/biz/pay/bizPayRecord/recovery?id=${bizPayRecord.id}&cendDele=end_dele" onclick="return confirmx('确认要恢复该交易记录吗？', this.href)">恢复</a>
					</c:if>
